...
CHECKPOINTS = {
    ...
    #"dsm": "vae/runs/celeba_dsm_20260610_135717/checkpoints/step_200000.pt",
...
def evaluate_one(vae_model, val_loader, mode):
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"Evaluating {mode.upper()}"):
            ...
            ce_metric.append( cross_entropy(recon, real).item() )
    ...
    ce_mean = np.mean(ce_metric)
    ce_var = np.std(ce_metric)
...
for mode, metrics in all_results.items():
    print(f"{mode.upper():<8} {metrics['rFID']:>8.2f} {metrics['PSNR']:>8.2f} {metrics['SSIM']:>8.3f} {metrics['CE_m']:>8.3f}({metrics['CE_v']:>8.3f})")
print(f"{'='*50}")
